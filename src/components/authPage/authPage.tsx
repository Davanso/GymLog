...
import { auth, authMessage } from '../../services/auth';
...
export function AuthPage({ serviceError = false }: { serviceError?: boolean }) {
  ...
  function reportAuthError(errorCode?: string) {
    if (!errorCode) return false;
    setError(authMessage(errorCode));
    return true;
  }
  async function login(email: string, password: string) {
    const result = await auth.signIn.email({ email, password });
    if (reportAuthError(result.error?.code)) return;
    window.location.assign('/app');
  }
  async function signup(email: string, password: string, values: FormData) {
    ...
    setNotice('Conta criada. Confira seu email para confirmar o cadastro e depois entre.');
  }
  async function forgotPassword(email: string) {
    ...
    setNotice('Se existir uma conta com esse email, você receberá um link para redefinir a senha.');
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const actions: Record<Mode, () => Promise<void>> = {
        ...
        signup: () => signup(email, password, values),
        forgot: () => forgotPassword(email),
        verify: () => verifyEmail(email),
        reset: () => resetPassword(password),
      };
      await actions[mode]();
    } catch {
