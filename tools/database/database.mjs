...
export async function migrate() {
  ...
  try {
    ...
    const applied = new Map(
      (await client.query('SELECT name, checksum FROM public.gymlog_migrations')).rows.map((r) => [
        r.name,
        r.checksum,
      ]),
    );
    ...
    for (const name of applied.keys())
      if (!files.includes(name)) throw new Error(`Applied migration missing: ${name}`);
    for (const name of files) {
      ...
      await client.query('INSERT INTO public.gymlog_migrations(name,checksum) VALUES ($1,$2)', [
        name,
        checksum,
      ]);
    ...
    console.error(
      `Migrations rolled back. ${error.code ? `SQLSTATE ${error.code}: ` : ''}${String(error.message).replace(/postgres(?:ql)?:\/\/\S+/g, '[REDACTED]')}`,
    );
    process.exitCode = 1;
  } finally {
    client.release();
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.error('Unable to connect for migrations.');
    process.exitCode = 1;
  }
}
