...
O sincronizador trabalha em lotes de quatro, reaproveita itens com vídeo e tenta cada detalhe até três vezes. Se o fornecedor limitar um detalhe, salva o resumo e a imagem; uma execução futura tenta completar o vídeo. A tabela versionada `exerciseNameTranslations.ts` garante nomes estáveis em português, enquanto a camada `exerciseTranslations.ts` localiza taxonomias e textos de instrução.
