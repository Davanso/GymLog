...
import { commitEmojis, normalizeMessage, validateHeader } from './commit-format.js'
...
test('preserves scopes, breaking marker, multiline body and trailers', () => {
  ...
  assert.equal(normalizeMessage(`feat(treinos)!: adicionar treino${body}`), `feat(treinos)!: ✨ adicionar treino${body}`)
...
test('rejects malformed headers and incorrect emojis', () => {
  for (const header of ['ajustes', 'unknown: ✨ alterar', 'feat: ✨ ', 'feat: 🐛 alterar', 'feat: ✨ alterar.', 'feat: ✨ ' + 'a'.repeat(100), 'Feat: ✨ alterar', 'feat(UPPER): ✨ alterar']) {
...
test('real Husky hook normalizes valid messages and rejects invalid ones without commits', () => {
  ...
  try {
    for (const [message, expected, valid] of [
      ...
      const result = spawnSync('git', ['hook', 'run', 'commit-msg', '--', path], { encoding: 'utf8' })
