...
try {
  const { rows } = await pool.query('SELECT name, applied_at FROM public.gymlog_migrations ORDER BY name');
...
} catch { console.error('Unable to read migration status. Configure the administrative connection and run db:migrate first.'); process.exitCode = 1; }
